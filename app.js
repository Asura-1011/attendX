...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Application State
  let currentStudent = AttendanceStore.getActiveUser();
  let currentTab = "overview"; // 'overview', 'predictor', 'timetable', 'history', 'credentials'
  let activeTimetableDay = getCurrentDayName();
  ...
  function showToast(message, type = "info") {
    ...
    if (cument.documentElement.setAttribute("data-theme", "light");
  }type === "danger") icon = "🚨";
  ...
  function renderApp() {
    ...
    // IF NOT LOGGED IN -> FORCE FULL PAGE LOGIN SCREEN
    if (!currentStudent) {
      renderLoginScreen();
      return;
    }
    ...
    appRoot.innerHTML = `
      <div class="app-container">
        ...
        <header class="navbar">
          <div class="navbar-inner">
            ...
            <div class="nav-actions">
              <!-- Active Student Profile Pill (Single User Lock) -->
        ...
        <main class="main-content">
          <!-- Shortage Warning Banner (High Priority) -->
        ...
        <div class="modal-box" onclick="event.stopPropagation()" style="max-width: 480px;">
          ...
          <form onsubmit="submitMarkCalendar(event, '${studentId}', '${subjectId}', ${isPresent})">
            ...
            <div class="form-group">
              ...
              <input type="text" id="mark-note-input" value="${escapeHtml(defaultNote)}" placeholder="e.g. Missed morning lecture" class="form-control" />
            </div>
  ...
  window.handleCreateSubject = function(e) {
    ...
    const created = AttendanceStore.addSubject(currentStudent.id, { code, name, credits, minPercentage, attended: 12, total: 12 });
    if (created) {
  ...
  window.confirmDeleteSubject = function(subjectId, name) {
    if (confirm(`Are you sure you want to remove ${name}?`)) {
      if (AttendanceStore.deleteSubject(currentStudent.id, subjectId)) {
